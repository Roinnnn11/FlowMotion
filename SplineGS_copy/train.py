...
GaussianModel = None
Scene = None
controlgaussians = None
dataset_readers = None
deformation = None
render_training_image = None
tqdm = None
safe_state = None
BasicPointCloud = None
getWorld2View2 = None
psnr = None
BinaryDiceLoss = None
l1_loss = None
    ...
    if stage == "warm":
        with torch.no_grad():
            ...
            if opt.use_instance_mask:
                ...
                total_mask_list = []
                if kornia is None:
                    raise ImportError("kornia is required for train.py warm-stage mask erosion.")
                for i in range(dyn_tracjectory.shape[1]):
                    current_traj = dyn_tracjectory[:, i, :]
                    X_norm = norm_dyn_tracklet[i, 0, :, 0]
                    Y_norm = norm_dyn_tracklet[i, 0, :, 1]
