...
def require_gsplat():
    if rasterization is None:
        raise ImportError("gsplat is required to use gaussian_renderer rendering functions.")
...
def render(
    ...
    means3D = dyn_pc.deform_points(viewpoint_camera.time)
...
def render_infer(viewpoint_camera,
    ...
    means3D = dyn_pc.deform_points(viewpoint_camera.time, inference=True)
    # Apply activations
    scales = dyn_pc.scaling_activation(scales)
    rotations = dyn_pc.rotation_activation(rotations)
