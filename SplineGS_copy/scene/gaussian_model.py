...
from scene.motion_model_factory import build_motion_model
...
class GaussianModel:
    ...
    def __init__(self, args):
        ...
        self.motion_model_type = getattr(args, "motion_model_type", "spline").lower()
        self.deform_spatial_scale = args.deform_spatial_scale
        self.error_threshold = args.prune_error_threshold
        self.motion_model = build_motion_model(args)
        self.current_control_num = nn.Parameter(
            torch.tensor([self.control_num], device="cuda", dtype=torch.int64), requires_grad=False
        )
        self.max_sh_degree = args.sh_degree
        self._xyz = torch.empty(0)
        self.control_xyz = nn.Parameter(torch.empty(0, device="cuda"), requires_grad=True)
    ...
    @property
    def control_xyz(self):
        return self.motion_model.control_xyz

    @control_xyz.setter
    def control_xyz(self, value):
        self.motion_model.control_xyz = value

    @property
    def current_control_num(self):
        return self.motion_model.current_control_num

    @current_control_num.setter
    def current_control_num(self, value):
        self.motion_model.current_control_num = value

    @property
    def flat_control_xyz(self):
        return self.motion_model.flat_control_xyz

    @flat_control_xyz.setter
    def flat_control_xyz(self, value):
        self.motion_model.flat_control_xyz = value

    @property
    def index_offset(self):
        return self.motion_model.index_offset

    @index_offset.setter
    def index_offset(self, value):
        self.motion_model.index_offset = value

    def deform_points(self, tau, inference=False):
        if inference:
    ...
    def flatten_control_point(self):
        self.motion_model.flatten_control_point()
    ...
    def onedown_control_pts(self, viewpoints):
        if self.motion_model_type != "spline":
            return
    ...
    def create_from_pcd_dynamic(
        ...
        self.motion_model.initialize_from_trajectory(self._xyz.detach(), dyn_tracjectory, self.deform_spatial_scale)
    ...
    def create_from_pcd(self, pcd: BasicPointCloud, spatial_lr_scale: float, time_line: int):
        ...
        self._xyz = nn.Parameter(fused_point_cloud.requires_grad_(True))
        self.motion_model.initialize_random(self._xyz.detach())
    ...
    def save_ply_compact_dy(self, path):
        ...
        # control_xyz = self.control_xyz.detach().flatten(start_dim=1).contiguous().cpu().numpy()
        current_control_num = self.current_control_num.detach().cpu().numpy()
        if self.flat_control_xyz.numel() == 0:
            self.flatten_control_point()
    ...
    def load_ply(self, path):
        ...
        control_x_names = [p.name for p in plydata.elements[0].properties if p.name.startswith("control_x_")]
        control_indices = sorted(int(name.split("_")[-1]) for name in control_x_names)
        control_xyz_list = []
        ...
        self.motion_model.control_num = control_xyz.shape[1]
    ...
    def load_ply_compact(self, path):
        ...
        if self.current_control_num.numel() != 0:
            self.motion_model.control_num = int(self.current_control_num.max().item())
        
        self.index_offset = (
            torch.cat(
                [torch.zeros(1).cuda(), torch.cumsum(self.current_control_num.reshape(-1)[:-1], dim=0)], dim=0
            ).long()
        )[:, None]
